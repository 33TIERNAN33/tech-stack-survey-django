# Full-Stack Web Application

This repository contains a full-stack web application built with Python,
Django, and SQLite. The application demonstrates server-side rendering
with Django Templates, responsive UI design using Bootstrap, automated
testing with Django’s built-in test framework, and debugging using the
VS Code Python debugger. Documentation is provided for setting up,
running, testing, and developing the application locally.

- [Development Guide (Docker)](dev-node/README.md)
- [Deployment Guide (Docker)](deploy-docker/README.md)

>[!TIP]
>Docker is the recommended approach for development and deployment as it abstracts away many of the complexities of server configuration and application setup. It allows you to run the application in a consistent environment across different machines and simplifies the deployment process. You are not required to use Docker for this course, but it is highly recommended for a smoother experience. If you choose to deploy manually without Docker, you will need to follow the manual deployment instructions provided in the documentation.

## Technology Stack

- Backend technology stack
    - Backend Language: [Python](https://www.python.org/)
    - Backend Framework: [Django](https://www.djangoproject.com/)
    - Web Server (development): [Django Development Server](https://docs.djangoproject.com/en/stable/ref/django-admin/#runserver) for local execution
    - Database: [SQLite](https://www.sqlite.org/index.html) for lightweight local data storage

- Frontend technology stack
    - Templates: [Django Templates](https://docs.djangoproject.com/en/stable/topics/templates/) for server-side rendering
    - UX/UI: [Bootstrap](https://getbootstrap.com/) for responsive styling and layout

- Testing Frameworks
    - Unit/Integration Testing: [Django Test Framework](https://docs.djangoproject.com/en/stable/topics/testing/) for automated request and unit tests

- Debugging Tools
    - Debugger: [VS Code Python Debugger (debugpy)](https://code.visualstudio.com/docs/python/debugging)
    - Logging: [Django Development Server Logs](https://docs.djangoproject.com/en/stable/ref/django-admin/#runserver)

## Manual Deployment

>[!WARNING]
>These instructions are for students who want to deploy the application manually without using Docker. This is an optional approach and is not required for the course. Manual deployment involves more steps and requires a deeper understanding of server configuration and application setup. It is recommended for students who want to gain hands-on experience with server administration and application deployment beyond the scope of Docker.

- [Deployment Guide (Manual)](deploy-node/README.md)
- [AWS EC2 Launch Guide](aws/README.md)

## Team Workflow

solo developer